...
svymean(~sleepWorkDays_bw, design = design_weighted) # 7.2877
svymean(~sleepWorkDays, design = design_weighted_2020) # 7.9761
# t-test
...
summary(model) # Multiple R-squared:  0.3049,	Adjusted R-squared:  0.2635
...
summary(model_weighted_age_sex) # Multiple R-squared:  0.3568,	Adjusted R-squared:  0.3185 
check_model(model_weighted_age_sex) # structurally bad PPC and residuals could be better, VIF high Age_group_merged
VIF(model_weighted_age_sex)
qqPlot(model_weighted_age_sex) # not so good in tail
...
# # Pseudo R²
# pseudo_R2 <- 1 - (rss / tss)
# pseudo_R2 # (=Multiple R^2 from lm + weights)
# -> check!
...
cor(df_coefs$weighted_coefs[-1], df_coefs$unweighted_coefs[-1]) # 0.90 without intercept
...
# sleepWorkDays_bw ~ education + marrital_status + work + min_sleep_required + 
#   napping + chronotype + insomniaChron + alcohol_intake + caffeine_intake + 
#   dinner_WorkDays

# estimate coefficients for this BE model:
BE_model_2017 <- lm(as.formula(k$model$terms), x = TRUE, y = TRUE, 
                    data = sleep17_sub)
summary(BE_model_2017) # Multiple R-squared:  0.284,	Adjusted R-squared:  0.2662
saveRDS(BE_model_2017, "./CURRENT_R_OUTPUTS/1_Results_with_ethnicity_education_optimScore/BE_model_2017_unweighted_2.4.25.RDS")
...
# "ethnicity"            "education"            "optimScore"           "marrital_status"      "BMI"                  "work"                 "min_sleep_required"   "napping"             
# "diagn_sleep_disorder" "insomniaChron"        "dinner_WorkDays"      "smoking"             

# estimate coefficients for this model:
BE_model_2017_weighted <- lm(terms(as.formula(k_w_sex_age)), 
                             data = sleep17_sub,
                             weights = weights(design_weighted))
summary(BE_model_2017_weighted) #Multiple R-squared:  0.3342,	Adjusted R-squared:  0.3168 
length(coef(BE_model_2017_weighted)) # 21
gtsummary::tbl_regression(BE_model_2017_weighted)
saveRDS(BE_model_2017_weighted, "./CURRENT_R_OUTPUTS/1_Results_with_ethnicity_education_optimScore/BE_model_2017_weighted_2.4.25.RDS")
check_model(BE_model_2017_weighted) # bump at 7 and 8h, fit could be better

# ____DROP in Adj.R^2 (->REPORT)-----------------
# from weighted full model to BE model: Adjusted R-squared:  0.3185 
# to weighted BE model Adjusted R-squared:  0.3168 
0.3185  - 0.3168  # 0.0017
# Difference in number of predictors full model vs BE model?
length(coef(model_weighted_age_sex)) # 45
length(coef(BE_model_2017_weighted)) # 21
45 - 21 # 24
...
for(i in 1:n_boot){
  ...
  model_boot <- lm(sleepWorkDays_bw ~ Age_group_merged + gender + 
                     ethnicity + education + optimScore +marrital_status +     
                     number_children + BMI + work + min_sleep_required + napping +       
                     chronotype + diagn_sleep_disorder + insomniaChron +  
                     alcohol_intake + caffeine_intake + dinner_WorkDays + 
                     sport_yes_no + smoking + general_health, 
...
toc() # 1400.143 sec elapsed; 2.4.25
...
quantile(adj.r.squared_w_as, p = c(0.025, 0.975)) #29-49%
...
plot(table(lengths(variables_used_list_sorted_w_as)), xlab = "number of predictors") # 7-19 predictors
...
incl_freq_2017_w_as$Variables <- gsub("ethnicity", "Ethnicity", incl_freq_2017_w_as$Variables)
incl_freq_2017_w_as$Variables <- gsub("education", "Education", incl_freq_2017_w_as$Variables)
incl_freq_2017_w_as$Variables <- gsub("optimScore", "Optimism Score", incl_freq_2017_w_as$Variables)
...
saveRDS(pred_boot_df_w_as, paste0(base_dir, "pred_boot_df_2017_w_as_2.4.25.RDS"))
saveRDS(variables_used_list_sorted_w_as, paste0(base_dir, "variables_used_list_sorted_2017_w_as_2.4.25.RDS"))
saveRDS(boot_strap_models_list_w_as, paste0(base_dir, "boot_strap_models_list_2017_w_as_2.4.25.RDS"))
saveRDS(adj.r.squared_w_as, paste0(base_dir, "adj.r.squared_boot_2017_weighted_2.4.25.RDS"))
saveRDS(count_VIF_w_as, paste0(base_dir, "count_VIF_2017_weighted_2.4.25.RDS"))
saveRDS(coefs_bootstrap_w_as, paste0(base_dir, "coefs_bootstrap_2020_w_as_2.4.25.RDS"))
saveRDS(incl_freq_2017_w_as, paste0(base_dir, "incl_freq_2017_weighted_as_2.4.25.RDS"))

save_as_html(flextable(incl_freq_2017_w_as) %>%
               set_caption("2017 inclusion frequencies (age and sex) weighted"), 
             path = "./RESULTS/1_Results_with_ethnicity_education_optimScore/Table_3_inclusion_frequencies_2017_weighted_2.4.25.html")

# READ:
# base_dir <- "/Users/juergen/Large_R_Files/1_Paper1/z_BOOTSTRAP_results/1_Results_with_ethnicity_education/2017_weighted/"
...
summary(model_full_2020) # Multiple R-squared:  0.2247,	Adjusted R-squared:  0.1821
...
summary(model_weighted_age_sex_2020) #Multiple R-squared:  0.2974,	Adjusted R-squared:  0.2588
check_model(model_weighted_age_sex_2020) # bad PPC and residuals, VIF -> question is if check_model correctly works with svyglm...
qqPlot(model_weighted_age_sex_2020) # not so normal

# potential interaction terms using RFA:
FSA_model <- lmFSA(sleepWorkDays ~ ., data = sleep20_sub)
summary(FSA_model)
# min_sleep_required:caffeine_intake #ns
...
summary(BE_model_2020) # Adjusted R-squared:  0.1726
...
summary(BE_model_2020_w_as) # Multiple R-squared:  0.2892,	Adjusted R-squared:  0.2622 
length(coef(BE_model_2020_w_as)) # 32
check_model(BE_model_2020_w_as) # bad PPC and residuals, obs 751?
qqPlot(BE_model_2020_w_as) # not so normal
gtsummary::tbl_regression(BE_model_2020_w_as) 
saveRDS(BE_model_2020_w_as, "./CURRENT_R_OUTPUTS/1_Results_with_ethnicity_education_optimScore/BE_model_2020_weighted_2.4.25.RDS")

# DROP in Adj.R^2 (->REPORT)-----------------
# from weighted full model to BE model: Adjusted R-squared:  0.2489 
# Adjusted R-squared:  0.2622 
0.2588 - 0.2622  # -0.0034
...
cor(df_coefs_2020$weighted_coefs, df_coefs_2020$unweighted_coefs)  # 0.94 with Intercept
cor(df_coefs_2020$weighted_coefs[-1], df_coefs_2020$unweighted_coefs[-1])  # 0.51!!!
# "outlier": workpart_time_inkl_NS
...
variables_used_list_sorted_w_as <- lapply(variables_used_list_w_as, sort)
hist(adj.r.squared_w_as)
quantile(adj.r.squared_w_as, p = c(0.025, 0.975)) # 0.23 0.44
...
incl_freq_2020_w_as$Variables <- gsub("optimScore", "Optimism Score", incl_freq_2020_w_as$Variables)
...
saveRDS(pred_boot_df_w_as, paste0(base_dir, "pred_boot_df_2020_w_as_2.4.25.RDS"))
saveRDS(variables_used_list_sorted_w_as, paste0(base_dir, "variables_used_list_sorted_2020_w_as_2.4.25.RDS"))
saveRDS(boot_strap_models_list_w_as, paste0(base_dir, "boot_strap_models_list_2020_w_as_2.4.25.RDS"))
saveRDS(adj.r.squared_w_as, paste0(base_dir, "adj.r.squared_boot_2020_weight_2.4.25.RDS"))
saveRDS(count_VIF_w_as, paste0(base_dir, "count_VIF_2020_weighted_2.4.25.RDS"))
saveRDS(coefs_bootstrap_w_as, paste0(base_dir, "coefs_bootstrap_w_as_2.4.25.RDS"))
saveRDS(incl_freq_2020_w_as, paste0(base_dir, "incl_freq_2020_weighted_age_sex_2.4.25.RDS"))

save_as_html(flextable(incl_freq_2020_w_as) %>%
               set_caption("2020 inclusion frequencies (age and sex) weighted"), 
             path = "./RESULTS/1_Results_with_ethnicity_education_optimScore/Table_3_inclusion_frequencies_2020_weighted_2.4.25.html")
...
summary(model_full_17_20) # Multiple R-squared:  0.2786,	Adjusted R-squared:  0.2581 
check_model(model_full_17_20) # PPC bad and residuals off
...
summary(model_full_17_20_weighted) # Multiple R-squared:  0.3188,	Adjusted R-squared:  0.2995
...
BE_model_17_20_weighted <- stepAIC(model_full_17_20_weighted, 
                                   direction = "backward", 
                                   trace = FALSE)
summary(BE_model_17_20_weighted) # Multiple R-squared:  0.3073,	Adjusted R-squared:  0.297
saveRDS(BE_model_17_20_weighted, "./CURRENT_R_OUTPUTS/1_Results_with_ethnicity_education_optimScore/BE_model_17_20_weighted_2.4.25.RDS")
...
tab$table_body$label <- gsub("_", " ", tab$table_body$label)
tab$table_body$label <- gsub("inkl", "incl.", tab$table_body$label)
tab$table_body$label <- gsub("exkl", "excl.", tab$table_body$label)
tab$table_body$label[tab$table_body$label == "university degree and above"] <- "University degree and above"
# Nur Werte ändern, keine Überschriften
tab$table_body$label[tab$table_body$label == "No chronic insomnia" & tab$table_body$row_type == "level"] <- "No"
tab$table_body$label[tab$table_body$label == "Chronic insomnia" & tab$table_body$row_type == "level"] <- "Yes"
tab <- tab %>%
  modify_header(label = "")
tab <- tab %>%
  bold_labels() %>%
  modify_header(label = "")  # still remove the column title
tab
# save manually
...
variables_used_list_sorted_w_as <- lapply(variables_used_list_w_as, sort)
hist(adj.r.squared_w_as)
quantile(adj.r.squared_w_as, p = c(0.025, 0.975)) # 0.26 0.41 
...
# ______Model sizes overview (number of predictors): ----
plot(table(lengths(variables_used_list_sorted_w_as)), xlab = "number of predictors") # 9-19
...
incl_freq_17_20_w_as$Variables <- gsub("gender", "Gender", incl_freq_17_20_w_as$Variables)
incl_freq_17_20_w_as$Variables <- gsub("ethnicity", "Ethnicity", incl_freq_17_20_w_as$Variables)
incl_freq_17_20_w_as$Variables <- gsub("education", "Education", incl_freq_17_20_w_as$Variables)
incl_freq_17_20_w_as$Variables <- gsub("optimScore", "Optimism Score", incl_freq_17_20_w_as$Variables)
...
base_dir <- "/Users/juergen/Large_R_Files/1_Paper1/z_BOOTSTRAP_results/1_Results_with_ethnicity_education_optimScore/2017_2020_combined/"
saveRDS(pred_boot_df_w_as, paste0(base_dir, "pred_boot_df_17_20_w_as_2.4.25.RDS"))
saveRDS(variables_used_list_sorted_w_as, paste0(base_dir, "variables_used_list_sorted_17_20_w_as_2.4.25.RDS"))
saveRDS(boot_strap_models_list_w_as, paste0(base_dir, "boot_strap_models_list_17_20_w_as_2.4.25.RDS"))
saveRDS(adj.r.squared_w_as, paste0(base_dir, "adj.r.squared_boot_17_20_weighted_2.4.25.RDS"))
saveRDS(incl_freq_17_20_w_as, paste0(base_dir, "incl_freq_17_20_weighted_2.4.25.RDS"))
saveRDS(coefs_bootstrap_w_as, paste0(base_dir, "coefs_bootstrap_17_20_weighted_2.4.25.RDS"))
saveRDS(count_VIF_w_as, paste0(base_dir, "count_VIF_17_20_weighted_2.4.25.RDS"))

save_as_html(flextable(incl_freq_17_20_w_as) %>%
               set_caption("2017 + 2020 inclusion frequencies weighted"), 
             path = "./RESULTS/1_Results_with_ethnicity_education_optimScore/Table_3_inclusion_frequencies_17_20_weighted_2.4.25.html")
...
model_weighted <- lm(sleepWorkDays ~ ., 
                     ...
                     weights = weights(design_weighted_2020))
summary(model_weighted) # Multiple R-squared:  0.3618,	Adjusted R-squared:  0.3104
check_model(model_weighted) # bad PPC and residuals suboptimal, VIF?
...
BE_model_2020_with_COVID_weighted <- stepAIC(model_weighted, 
                                             direction = "backward", 
                                             trace = FALSE)
summary(BE_model_2020_with_COVID_weighted) # Multiple R-squared:  0.3364,	Adjusted R-squared:  0.3112
check_model(BE_model_2020_with_COVID_weighted) # PPC and residuals suboptimal; 751?

saveRDS(BE_model_2020_with_COVID_weighted, file = "./CURRENT_R_OUTPUTS/1_Results_with_ethnicity_education_optimScore/BE_model_2020_with_COVID_weighted_2.4.25.rds")

# DROP in R-squared:

# Full model: Adjusted R-squared:  0.3097
# BE model weighted: Adjusted R-squared:  0.3112
0.3104 -  0.3112 # -8e-04
...
for(i in 1:n_boot){
  ...
  model_boot_w <- lm(sleepWorkDays ~ Age_group_merged + gender + 
                         ...
                         weights = weights(design_2020_boot))
  k_w_as <- stepAIC(model_boot_w, direction = "backward", trace = FALSE) 
  count_VIF_w_as <- count_VIF_w_as + sum(VIF(model_boot_w)[,3] > 3)   # multicollinearity?
  boot_strap_models_list_w_as[[i]] <- k_w_as
  variables_used_w_as <- labels(as.formula(k_w_as$terms))
  variables_used_list_w_as[[i]] <- sort(variables_used_w_as)
  predictors_boot_w_as[i] <- paste(sort(variables_used_w_as), collapse = " ")
...
# ______Model sizes overview (number of predictors): ----
plot(table(lengths(variables_used_list_sorted_w_as)), xlab = "number of predictors") # 10-26
...
incl_freq_2020$Variables <- gsub("optimScore", "Optimism Score", incl_freq_2020$Variables)
...
saveRDS(variables_used_list_sorted_w_as, paste0(base_dir, "variables_used_list_sorted_2020_w_as_2.4.25.RDS"))
saveRDS(boot_strap_models_list_w_as, paste0(base_dir, "boot_strap_models_list_2020_w_as_2.4.25.RDS"))
saveRDS(count_VIF_w_as, paste0(base_dir, "count_VIF_2020_w_as_2.4.25.RDS"))
saveRDS(adj.r.squared_w_as, paste0(base_dir, "adj.r.squared_2020_w_as_2.4.25.RDS"))
saveRDS(coefs_bootstrap_w_as, paste0(base_dir, "coefs_bootstrap_2020_w_as_2.4.25.RDS"))
saveRDS(incl_freq_2020, paste0(base_dir, "incl_freq_2020_weighted_incl_COVID_2.4.25.RDS"))

save_as_html(flextable(incl_freq_2020) %>%
               set_caption("2020 incl COVID inclusion frequencies (age and sex) weighted"),
             path = "./RESULTS/1_Results_with_ethnicity_education_optimScore/Table_3_inclusion_frequencies_2020_incl_COVID_weighted_2.4.25.html")
