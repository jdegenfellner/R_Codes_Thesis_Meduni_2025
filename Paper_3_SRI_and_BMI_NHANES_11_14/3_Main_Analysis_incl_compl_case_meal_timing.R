# 3_Main_Analysis_incl_compl_case_meal_timing
...
(current_directory <- getwd())
(parent_directory <- dirname(current_directory))
saveRDS(df_an, paste0(parent_directory, "/4_DATA/Combined_data_sets_2_waves_analytic_data_set_15.4.25_incl_meal_timing.RDS"))
...
df_long_1112 <- df %>%
  dplyr::select(
    SEQN,
    starts_with("DR1DAY_11_12"),
    starts_with("first_meal_time_11_12"),
    starts_with("last_meal_time_11_12"),
    starts_with("eating_window_hours_11_12")
  ) %>%
  dplyr::mutate(Wave = "NHANES_1112") %>%
  dplyr::rename_with(
    ~ gsub("_11_12", "", .x),  # Nur Jahresinfo entfernen, nicht "day_1"
    .cols = -c(SEQN, Wave)
  )
...
df_long_1314 <- df %>%
  dplyr::select(
    SEQN,
    starts_with("DR1DAY_13_14"),
    starts_with("first_meal_time_13_14"),
    starts_with("last_meal_time_13_14"),
    starts_with("eating_window_hours_13_14")
  ) %>%
  dplyr::mutate(Wave = "NHANES_1314") %>%
  dplyr::rename_with(
    ~ gsub("_13_14", "", .x),
    .cols = -c(SEQN, Wave)
  )

# Combine long-format meal timing data-------
df_mealtimes_long <- dplyr::bind_rows(df_long_1112, df_long_1314)
...
# Left join to df_an1 and create unified meal timing vars for Day 1
df_an1 <- df_an1 %>%
  dplyr::left_join(df_mealtimes_long, by = c("SEQN", "Wave")) %>%
  dplyr::mutate(
    first_meal_time = as.numeric(first_meal_time_day_1),
    last_meal_time = as.numeric(last_meal_time_day_1),
    eating_window_hours = as.numeric(eating_window_hours_day_1)
  )
df_an1
dim(df_an1) # 7085 45
...
vis_miss(df_an1 %>% dplyr::select(contains("meal"), contains("eating"), contains("fasting")) %>% sample_n(1000)) +
  theme(plot.margin = unit(c(1, 2, 1, 1), "cm")) +  # Increase margins (top, right, bottom, left)
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

df_an1 %>%
  dplyr::select(contains("meal"), contains("eating"), contains("fasting")) %>%
  naniar::gg_miss_var(show_pct = TRUE)
# ~ 38% missing

colnames(df_an1)

# Conwert - otherwise tibble error...
df_an1 <- df_an1 %>%
  dplyr::mutate(
    first_meal_time_day_1 = as.numeric(first_meal_time_day_1),
    first_meal_time_day_2 = as.numeric(first_meal_time_day_2),
    last_meal_time_day_1  = as.numeric(last_meal_time_day_1),
    last_meal_time_day_2  = as.numeric(last_meal_time_day_2)
  )
...
vis_miss(
  df_an1 %>% dplyr::select(
    dplyr::contains("first_meal_time"),
    dplyr::contains("last_meal_time"),
    dplyr::contains("eating_window_hours"),
    dplyr::starts_with("DR1DAY_"),
    dplyr::starts_with("DR2DAY_")
  )
) +
...
df_an1_compl_meal <- dplyr::filter(
  df_an1,
  !is.na(first_meal_time_day_1) &
    !is.na(first_meal_time_day_2) &
    !is.na(last_meal_time_day_1) &
    !is.na(last_meal_time_day_2) &
    !is.na(eating_window_hours_day_1) &
    !is.na(eating_window_hours_day_2)
)
...
# impute using kNN--------
tic()
df_an1_imputed <- kNN(df_an1_compl_meal, k = 5)
toc() # 143
...
(current_directory <- getwd())
(parent_directory <- dirname(current_directory))
saveRDS(df_an1_imputed, paste0(parent_directory, "/4_DATA/df_an1_imputed_incl_meal_timing_16.4.25.RDS"))
...
# {Age, Sex, Ethnicity,
# Education, H.H.income, Occup(lastweek), Depression, Mar.Status, 
# Physical activity, Alcoh, Smoking, 25-hydroxyvitamin D}
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_BMI_strat_by_Sex_and_Ethnicity_BMI_without_other_covar_wins_sub_mealtiming_compl_without_meal_cov_15.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_BMI_strat_by_Sex_and_Ethnicity_BMI_wins_sub_mealtiming_compl_without_meal_cov_15.4.25.pdf")
ggsave(file_path)
...
anova(model3_svy, model6_svy, test = "Chisq") # small p= 0.00069054   # no interaction compared to both interactions: sex*SRI + ethnicity*SRI
anova(model4_svy, model6_svy, test = "Chisq") # small p= 0.017548 # interaction sex*SRI compared to both interactions: sex*SRI + ethnicity*SRI
anova(model5_svy, model6_svy, test = "Chisq") # small p= 0.001991  # interaction for ethnicity*SRI compared to both interactions: sex*SRI + ethnicity*SRI
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_via_sampling_sub_mealtiming_compl_cases_no_meal_cov_16.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_OTHER_adiposity_measures_WC_predictions_via_sampling_sub_meal_compl_meal_no_cov_meal_15.4.25.pdf")
ggsave(file_path)
