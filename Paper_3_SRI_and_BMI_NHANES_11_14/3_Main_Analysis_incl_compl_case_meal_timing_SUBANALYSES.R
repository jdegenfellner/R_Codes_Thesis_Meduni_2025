# 3_Main_Analysis_incl_compl_case_meal_timing_SUBANALYSES.R
...
df <- readRDS(paste0(parent_directory, "/4_DATA/Combined_data_sets_2_waves_15.4.25_incl_meal_timing.RDS"))
str(df)
dim(df) # 19931 773 check
...
#df_an$first_meal_time_11_12_day_1

# _SAVE analytic data set----
(current_directory <- getwd())
(parent_directory <- dirname(current_directory))
saveRDS(df_an, paste0(parent_directory, "/4_DATA/Combined_data_sets_2_waves_analytic_data_set_17.4.25_incl_meal_timing.RDS"))
...
# ____NEW: Meal timing variables ------

# READ meal timing variables from diet_days file
(parent_directory <- dirname(getwd()))
diet_days <- readRDS(file = file.path(parent_directory, 
                                      "z_intermediate_results", "diet_days_17.4.25.rds"))

vis_miss(
  diet_days %>% 
    dplyr::filter(SEQN %in% df_an1$SEQN) %>%
    dplyr::select(
    latest_meal_diff_hours,
    eating_window_difference
  )
) +
  theme(plot.margin = unit(c(1, 2, 1, 1), "cm")) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

# add to meal timing to df_an1
df_an1 <- df_an1 %>%
  dplyr::left_join(diet_days,
    by = "SEQN"
  )

# IMPUTE?--------------------
# -> NO, too many missings.
# - Create main analysis again on subset of complete meal timings
# - add meal-timing variables to models and study changes.

# Complete cases (meal-timing)---------
df_an1_compl_meal <- df_an1 %>%
  dplyr::filter(
    !is.na(eating_window_difference_wins), # complete in these two covariables
    !is.na(latest_meal_diff_hours_wins)
  ) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(across(everything(), identity)) # force rebuild
dim(df_an1_compl_meal) # 4392
dim(df_an1) # 7085
4392/7085
# -> NOW impute as before and do analyses

# 4) Impute missing values----
# check missings
vis_miss(df_an1_compl_meal %>% dplyr::select(-c(ALQ101, ALQ110, ALQ130, ALQ120Q, 
                                     ALQ120U, SMQ040, SMQ020, SMD650)) %>% 
           sample_n(1000))  +
  theme(plot.margin = unit(c(1, 2, 1, 1), "cm")) +  # Increase margins (top, right, bottom, left)
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
df_an1_compl_meal %>% dplyr::select(-c(ALQ101, ALQ110, ALQ130, ALQ120Q, ALQ120U, 
...
vis_miss(df_an1_compl_meal %>% dplyr::select(-c(ALQ101, ALQ110, ALQ130, 
...
# impute using kNN--------
tic()
df_for_knn <- df_an1_compl_meal |>
  dplyr::select(where(~ !inherits(.x, "Period"))) |> # remove all date/time variables, otherwise error in kNN
  as.data.frame()
df_an1_imputed <- VIM::kNN(df_for_knn, k = 5, imp_var = FALSE)
toc() # 143
vis_miss(df_an1_imputed %>% sample_n(1000)) # check, worked
df_an1_imputed %>%
  dplyr::summarise(across(everything(), ~ mean(is.na(.)) * 100)) %>%
  dplyr::select(where(~ . > 0))

# add meal timing variables back again:
df_an1_imputed <- df_an1_imputed %>%
  dplyr::left_join(
    diet_days %>%
      dplyr::select(SEQN, eating_window_difference_wins, latest_meal_diff_hours_wins),
    by = "SEQN"
  )
df_an1_imputed$eating_window_difference_wins # check
df_an1_imputed$latest_meal_diff_hours_wins # check
vis_miss(df_an1_imputed %>% sample_n(1000)) # worked.

# clean up variables from merging:
df_an1_imputed <- df_an1_imputed %>%
  dplyr::select(-latest_meal_diff_hours_wins.x,
                -eating_window_difference_wins.x,
    -latest_meal_diff_hours_wins.y,
    -eating_window_difference_wins.y
  )

# save:
setwd(dirname(getwd()))
saveRDS(df_an1_imputed, file = "./RESULTS/df_an1_imputed_before_modeling_incl_mealtiming_22.4.25.rds")
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))
getwd()
...
(current_directory <- getwd())
(parent_directory <- dirname(current_directory))
saveRDS(df_an1_imputed, paste0(parent_directory, "/4_DATA/df_an1_imputed_incl_meal_timing_19.4.25.RDS"))
...
# First, all models will be run as before, just the subset has no changed
# to the complete cases (with imputation of the rest) of meal timing.

# Second, meal timing variables will be added to the models
...
# _imputed---------
# TODO add to diet_days, for now, winsorize here:
df_an1_imputed$latest_meal_diff_hours_wins <- DescTools::Winsorize(df_an1_imputed$latest_meal_diff_hours, 
                                                                   val = quantile(df_an1_imputed$latest_meal_diff_hours, probs = c(0.01, 0.99), na.rm = TRUE))
df_an1_imputed$eating_window_difference_wins <- DescTools::Winsorize(df_an1_imputed$eating_window_difference,
                                                                     val = quantile(df_an1_imputed$eating_window_difference, probs = c(0.01, 0.99), na.rm = TRUE))
...
#svymean(~latest_meal_diff_hours, nhanesDesign) # 0.017675
#svymean(~eating_window_difference, nhanesDesign) # -0.40378
...
check_model(model1) # not bad, residuals could be better,
...
model2 <- lm(log(BMXBMI) ~ SRI_no_imputation_winsorized_rand + RIDAGEYR + RIAGENDR_factor, 
             data = df_an1_imputed)
summary(model2)
check_model(model2) # all green, residuals could be better, but not so bad
qqPlot(model2, main = "QQ Plot of Model 2", envelope = TRUE) # could be better, not so bad
...
# REPORT MODEL 1, table 3 svy IN PAPER--------
...
model3 <- lm(log(BMXBMI) ~ 
              ...
              #LBDHDDSI,                    # Direct HDL Cholesterol
              latest_meal_diff_hours_wins + # latest meal difference Mo-Do and Fr-So
              eating_window_difference_wins, # eating window difference Mo-Do and Fr-So
            data = df_an1_imputed)
summary(model3) # mealtiming ns.

check_model(model3) # all green
...
ggplot(pred, aes(x = x, y = exp(predicted))) +
  ...
  theme_minimal() + 
  theme(plot.title = element_text(hjust = 0.5))
# Decreasing relationship: ~2.5 points over SRI-range
...
# REPORT Model 2 (table 3) in paper---------
...
# -> makes sense, since other covariables explain something as well.
...
# Male:
tic()
pred_svy <- bootstrap_predictions_svyglm_incl_mealtiming_vars(model3_svy, df_an1_imputed, 
                                         SRI_range, Sex = "Male", n_boot = 300)
toc() # ~ 20s
ggplot(pred_svy, aes(x = SRI_no_imputation_winsorized_rand, y = exp(Predicted_BMI))) +
  geom_line(color = "blue", size = 1) +
...
sex_levels <- c("Male", "Female")
tic()
pred_svy_sex <- bootstrap_predictions_svyglm_sex_vary_incl_mealtiming_vars(model3_svy, df_an1_imputed, 
                                                      SRI_range, sex_levels, n_boot = 300)
toc() # 
pred_svy_sex
ggplot(pred_svy_sex, aes(x = SRI_no_imputation_winsorized_rand, 
                         y = exp(Predicted_BMI), color = RIAGENDR_factor)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = exp(CI_Lower), ymax = exp(CI_Upper), 
                  fill = RIAGENDR_factor), alpha = 0.2) +
...
# COMPARE effect sizes for model 3 (with vs. without survey weights)-------
...
# Male:
tic() 
pred_svy_4 <- bootstrap_predictions_svyglm_incl_mealtiming_vars(model4_svy, 
                                                                df_an1_imputed, 
                                        SRI_range, Sex = "Male", n_boot = 300)
toc() # ~ 20s
...
tic() 
sex_levels <- c("Male", "Female")
pred_svy <- bootstrap_predictions_svyglm_sex_vary_incl_mealtiming_vars(model4_svy, 
                                                                       df_an1_imputed, 
...
ggplot(pred_svy, aes(x = SRI_no_imputation_winsorized_rand, y = exp(Predicted_BMI), color = RIAGENDR_factor)) +
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  facet_wrap(~RIAGENDR_factor)
# -> Both decreasing, Female ~ 5 points MUCH stronger, male very slightly decreasing
...
model5_svy <- svyglm(log(BMXBMI) ~ 
                     ...
                     design = nhanesDesign)
summary(model5_svy)
qqPlot(model5_svy$residuals, main = "QQ Plot of Weighted Model", 
       envelope = TRUE) # a bit worse than unweighted, not so normal
# bootstrap CIs:
#tic()
...
tic() 
sex_levels <- c("Male", "Female")
pred_svy <- bootstrap_predictions_svyglm_sex_vary_incl_mealtiming_vars(model5_svy, df_an1_imputed, 
                                                  SRI_range, sex_levels, n_boot = 300)
toc() # 48s
pred_svy
ggplot(pred_svy, aes(x = SRI_no_imputation_winsorized_rand, y = exp(Predicted_BMI), color = RIAGENDR_factor)) +
...
tic()
bootstrap_summary <- bootstrap_predictions_svyglm_ethnicity_varied_incl_mealtiming(model5_svy, 
                                                                   df_an1_imputed, 
                                                                   SRI_range, 
...
SRI_range <- seq(0, 100, by = 5)
tic()
bootstrap_summary <- bootstrap_predictions_svyglm_sex_ethnicity_varied_incl_mealtiming_vars(model3_svy, 
                                                                       df_an1_imputed, 
                                                                       SRI_range, sex_levels, 
                                                                       ethnicity_levels, n_boot = 100)
toc() # 40s
...
tic()
bootstrap_summary_BMI_without_interaction <- bootstrap_predictions_svyglm_sex_ethnicity_varied_incl_mealtiming_vars(
  model6_svy_without_adjustments_only_2_interactions, 
  df_an1_imputed, 
  SRI_range, 
  sex_levels, 
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_BMI_strat_by_Sex_and_Ethnicity_BMI_without_other_covar_wins_sub_mealtiming_compl_WITH_meal_cov_22.4.25.pdf")
ggsave(file_path)
...
# _Model assumptions (Model 6 with weights):----------
...
cor(pred_df$unweighted_predictions, pred_df$weighted_predictions) # 0.97
...
source("./z_stuff/bootstrap_prediction_CIs_for_svy_models.R")
tic()
bootstrap_summary <- bootstrap_predictions_svyglm_sex_ethnicity_varied_incl_mealtiming_vars(model6_svy, 
                                                                       df_an1_imputed, 
                                                                       SRI_range, sex_levels, 
...
ggplot(bootstrap_summary, aes(x = SRI_no_imputation_winsorized_rand, 
                              y = exp(Predicted_BMI), 
                              color = RIAGENDR_factor, 
                              fill = RIAGENDR_factor)) +
  geom_line(size = 1) +  # Line for predicted BMI
  geom_ribbon(aes(ymin = exp(CI_Lower), ymax = exp(CI_Upper)), 
              alpha = 0.3, color = NA) +  # Shaded confidence interval
  facet_wrap(~RIDRETH3_factor, scales = "fixed") +  # Facet by ethnicity only
  labs(title = "Predicted BMI by SRI, Stratified by Ethnicity and Sex",
       x = "SRI (Sleep Regularity Index)",
       y = "Predicted BMI",
  ...
  scale_color_manual(values = c("Male" = "blue", 
                                "Female" = "red")) +  # Custom colors for sexes
  scale_fill_manual(values = c("Male" = "blue", 
                               "Female" = "red")) +  # Matching fill colors
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 14),
    axis.title = element_text(size = 12),
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_BMI_strat_by_Sex_and_Ethnicity_BMI_wins_sub_mealtiming_compl_WTIH_meal_cov_18.4.25.pdf")
ggsave(file_path)
...
anova(model3_svy, model6_svy, test = "Chisq") # small p= 0.00058158 # no interaction compared to both interactions: sex*SRI + ethnicity*SRI
anova(model4_svy, model6_svy, test = "Chisq") # small p= 0.019762 # interaction sex*SRI compared to both interactions: sex*SRI + ethnicity*SRI
anova(model5_svy, model6_svy, test = "Chisq") # small p= 0.0034488   # interaction for ethnicity*SRI compared to both interactions: sex*SRI + ethnicity*SRI
...
param_space <- data.frame(
  ...
  latest_meal_diff_hours_wins = sample(df_an1_imputed$latest_meal_diff_hours_wins, n_samples, replace = TRUE),  # Latest meal difference
  eating_window_difference_wins = sample(df_an1_imputed$eating_window_difference_wins, n_samples, replace = TRUE)  # Eating window difference
)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_via_sampling_sub_mealtiming_compl_cases_WITH_meal_cov_16.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_OTHER_adiposity_measures_WC_predictions_via_sampling_sub_meal_compl_meal_WITH_cov_meal_15.4.25.pdf")
ggsave(file_path)
...
# weighted median meal timing variables using svymean
svymean(~latest_meal_diff_hours_wins, nhanesDesign, na.rm = TRUE) # 0.025083
svymean(~eating_window_difference_wins, nhanesDesign, na.rm = TRUE) # -0.38433
...
# Run bootstrapped predictions for Waist Circumference
tic()
bootstrap_summary_WC <- bootstrap_predictions_svyglm_sex_ethnicity_varied_incl_mealtiming_vars(
  model7_svy, 
  df_an1_imputed, 
  SRI_range, 
  sex_levels, 
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_WC_strat_by_Sex_and_Ethnicity_WC_compl_meal_no_meal_cov_15.4.25.pdf")
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_OTHER_adiposity_measures_ABSI_predictions_via_sampling_compl_meal_no_meal_cov_15.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_ABSI_strat_by_Sex_and_Ethnicity_compl_meal_no_meal_cov_ABSI_15.4.25.pdf")
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_OTHER_adiposity_measures_WHtR_predictions_via_sampling_compl_meal_no_meal_cov_15.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_WHtR_strat_by_Sex_and_Ethnicity_WHtR_compl_meal_no_meal_cov_15.4.25.pdf")
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_OTHER_adiposity_measures_ASAD_predictions_via_sampling_compl_meal_no_meal_cov_15.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_SAD_strat_by_Sex_and_Ethnicity_SAD_comp_meal_no_meal_cov_15.5.25.pdf")
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_OTHER_adiposity_measures_VAI_predictions_via_sampling_compl_meal_no_meal_cov_15.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_VAI_strat_by_Sex_and_Ethnicity_VAI_compl_meal_no_meal_cov_15.4.25.pdf")
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_OTHER_adiposity_measures_TOTFAT_predictions_via_sampling_compl_meal_no_meal_cov_15.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_TOTFAT_strat_by_Sex_and_Ethnicity_DXDTOFAT_compl_meal_no_meal_cov_15.4.25.pdf")
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_OTHER_adiposity_measures_FMI_predictions_via_sampling_compl_meal_no_meal_cov_15.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_FMI_strat_by_Sex_and_Ethnicity_compl_meal_no_meal_cov_FMI_15.4.25.pdf")
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_OTHER_adiposity_measures_BRI_predictions_via_sampling_compl_meal_no_meal_cov_15.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_BRI_strat_by_Sex_and_Ethnicity_BRI_compl_meal_no_meal_cov_15.4.25.pdf")
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_OTHER_adiposity_measures_SADHtR_predictions_via_sampling_compl_meal_no_meal_cov_15.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_SADHtR_strat_by_Sex_and_Ethnicity_SADHtR_compl_meal_no_meal_cov_15.4.25.pdf")
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_OTHER_adiposity_measures_LAP_predictions_via_sampling_compl_meal_no_meal_cov_15.4.25.pdf")
ggsave(file_path)
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_LAP_strat_by_Sex_and_Ethnicity_LAP_compl_meal_no_meal_cov_15.4.25.pdf")
...
file_path <- file.path(parent_directory, 
                       ...
                       "Figures", 
                       "SUB_mealtiming",
                       "Model_2_predictions_DXDTOPF_strat_by_Sex_and_Ethnicity_DXDTOPF_compl_meal_no_meal_cov_15.4.25.pdf")
...
(current_directory <- getwd())
(parent_directory <- dirname(current_directory))
output_path <- "/1_Main_Document/Figures_Supplement/SRI_distribution_15.4.15_compl_meal.pdf"
ggsave(filename = paste0(parent_directory,output_path), plot = p2, device = "pdf", width = 8, height = 6) 
...
# Abstract----
# calculations for overfall female effect:
...
calculate_overall_effect_females <- function(model, SRI_start, SRI_end) {
  fixed_values <- data.frame(
    ...
    RIAGENDR = 2  # Set to FEMALE for predictions
  )
  
  # Predict log(BMI) for start and end SRI values
...
bootstrap_overall_effects_females <- function(data, n_boot = 500, SRI_medians) {
  ...
  for (i in 1:n_boot) {
    ...
    boot_model <- svyglm(log(BMXBMI) ~ SRI_no_imputation_winsorized_rand + RIDAGEYR + RIAGENDR, 
                         design = boot_design)
...
tic()
bootstrap_results_females <- bootstrap_overall_effects_females(df_an1_imputed, n_boot = 500, SRI_medians)
toc() # 12s
